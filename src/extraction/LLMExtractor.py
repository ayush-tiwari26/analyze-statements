...
class LLMExtractor(Extractor):
    ...
    def extract_single(self, content: str) -> Dict:
        ...
        prompt = """
            ...
            ■ JSON object 
            ■ Fields: 
                1. `starting_balance`: the starting balance of the statement. Value MUST be a 2 decimal double. No currency signs
                2. `ending_balance`: the ending balance. Value MUST be a 2 decimal double. No currency signs
                3. `transactions`: a list of json objects where each object has fields
                    ...
                    3.3 `amount`: The monetary value of the transaction. MUST be positive & MUST be a 2 decimal double. No currency signs
                    3.4 `direction: Indicate whether the transaction is a `debit` or `credit` (always as string in small case).
